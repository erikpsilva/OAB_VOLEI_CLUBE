...
// Verifica duplicidade de e-mail/CPF em outro jogador
$stmt = $pdo->prepare("SELECT id FROM jogadores WHERE (email = ? OR cpf = ?) AND id != ? LIMIT 1");
...
if ($senha !== '') {
    ...
    $stmt = $pdo->prepare("
        UPDATE jogadores
        SET nome_completo = ?, email = ?, cpf = ?, telefone = ?, data_nascimento = ?, senha = ?
        WHERE id = ?
    ");
    $stmt->execute([$nomeCompleto, $email, $cpf, $telefone, $dataNascimento ?: null, $hash, $id]);
} else {
    $stmt = $pdo->prepare("
        UPDATE jogadores
        SET nome_completo = ?, email = ?, cpf = ?, telefone = ?, data_nascimento = ?
        WHERE id = ?
    ");
    $stmt->execute([$nomeCompleto, $email, $cpf, $telefone, $dataNascimento ?: null, $id]);
}
